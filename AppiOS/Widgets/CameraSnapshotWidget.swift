...
/// Home Screen widget: latest cached snapshot from one camera + the
/// last-motion-event timestamp. Configurable via `SelectCameraIntent`.
...
public struct CameraSnapshotProvider: AppIntentTimelineProvider {
    ...
    public func timeline(for configuration: SelectCameraIntent, in context: Context) async -> Timeline<CameraSnapshotEntry> {
        ...
        // Refresh on app-driven updates (which schedule
        // `WidgetCenter.reloadAllTimelines()` after each preview
        // write). The 15-minute fallback ensures stale displays
        // self-recover when the main app hasn't been opened.
...
struct CameraSnapshotView: View {
    ...
    var body: some View {
        ZStack(alignment: .bottomLeading) {
            if let data = entry.snapshotImageData, let uiImage = UIImage(data: data) {
